...
def find_user_id_by_username(username: str) -> str | None:
    needle = str(username or "").strip().lstrip("@").lower()
    if not needle:
        return None
    ws = _ws(USERS_WS)
    for row in ws.get_all_records():
        if str(row.get("username", "")).strip().lstrip("@").lower() == needle:
            return str(row.get("id", "")).strip() or None
    return None


def get_user_snapshot(user_ref: str) -> dict | None:
    ref = str(user_ref or "").strip()
    if not ref:
        return None
    if ref.startswith("@"):
        resolved = find_user_id_by_username(ref)
        if not resolved:
            return None
        ref = resolved
    ...
    for row in ws.get_all_records():
        if str(row.get("id", "")).strip() == ref:
            expires = _to_float(row.get("sub_expires_at") or row.get("unsubscribed_at") or "")
            return {
                "id": str(row.get("id", "")).strip(),
                "username": str(row.get("username", "")).strip(),
                "subscribed": _to_bool(row.get("subscribed", "")),
                "sub_expires_at": expires,
                "ref_code": str(row.get("ref_code", "")).strip(),
                "ref_from": str(row.get("ref_from", "")).strip(),
                "sub_cancelled": _to_bool(row.get("sub_cancelled", "")),
                "payment_id": str(row.get("payment_id", "")).strip(),
                "subscription_order_reference": str(row.get("subscription_order_reference", "")).strip(),
                "wayforpay_recurring_status": str(row.get("wayforpay_recurring_status", "")).strip(),
            }
    return None
...
def set_subscription(user_id: str, active: bool, price_uah: int = 0,
                     period_text: str = "1m", payment_id: str = "",
    ...
    if active:
        ...
        if order_reference:
            order_ref_col = _ensure_user_column(ws, "subscription_order_reference")
            ws.update_acell(_a1(row_idx, order_ref_col), order_reference)
        recurring_status_col = _ensure_user_column(ws, "wayforpay_recurring_status")
        ws.update_acell(_a1(row_idx, recurring_status_col), "active")
    else:
        ws.spreadsheet.values_batch_update({
            "valueInputOption": "RAW",
            "data": [
...
def mark_wayforpay_recurring_status(user_id: str, status: str) -> None:
    ws = _ws(USERS_WS)
    row_idx = _find_row_by_user_id(ws, str(user_id))
    if not row_idx:
        return
    status_col = _ensure_user_column(ws, "wayforpay_recurring_status")
    ws.update_acell(_a1(row_idx, status_col), str(status or "").strip())


def get_subscription_gateway_meta(user_id: str) -> dict:
    ws = _ws(USERS_WS)
    row_idx = _find_row_by_user_id(ws, str(user_id))
    if not row_idx:
        return {"payment_id": "", "order_reference": "", "sub_cancelled": False}
    ...
    payment_id = _cell(12)  # L
    order_reference = _cell(order_ref_col)
    if not order_reference and payment_id.startswith("FF-"):
        order_reference = payment_id

    return {
        "payment_id": payment_id,
        "order_reference": order_reference,
        "sub_cancelled": _to_bool(_cell(13)),
        "wayforpay_recurring_status": _cell(recurring_status_col),
    }


def _favorite_key(card: dict) -> str:
    return str(
        card.get("_key")
        or card.get("link")
...
def get_user_favorites(user_id: str) -> list[dict]:
    ...
    favorites_col = _ensure_user_column(ws, "favorites_json")
    row = ws.row_values(row_idx)
    raw = str(row[favorites_col - 1]).strip() if len(row) >= favorites_col else ""
    if not raw:
        return []
    try:
        data = json.loads(raw)
...
def _write_user_favorites(ws, row_idx: int, items: list[dict]) -> None:
    favorites_col = _ensure_user_column(ws, "favorites_json")
    ws.update_acell(_a1(row_idx, favorites_col), json.dumps(items[:100], ensure_ascii=False))


def toggle_user_favorite(user_id: str, card: dict) -> dict:
    ws = _ws(USERS_WS)
    row_idx = _find_row_by_user_id(ws, str(user_id))
    if not row_idx:
        ensure_user(str(user_id), "")
    ...
    items = get_user_favorites(str(user_id))
    key = _favorite_key(card)
    if not key:
        return {"saved": False, "count": len(items)}

    existing_idx = next((idx for idx, item in enumerate(items) if str(item.get("key", "")).strip() == key), None)
    if existing_idx is not None:
        items.pop(existing_idx)
        _write_user_favorites(ws, row_idx, items)
        return {"saved": False, "count": len(items), "key": key}

    image_url = ""
    img_urls = card.get("img_urls") or []
    if img_urls:
        image_url = str(img_urls[0]).strip()
    ...
    items.insert(0, item)
    _write_user_favorites(ws, row_idx, items)
    return {"saved": True, "count": len(items), "key": key}


def remove_user_favorite(user_id: str, key: str) -> bool:
    ws = _ws(USERS_WS)
    row_idx = _find_row_by_user_id(ws, str(user_id))
    if not row_idx:
        return False
    items = get_user_favorites(str(user_id))
    before = len(items)
    items = [item for item in items if str(item.get("key", "")).strip() != str(key or "").strip()]
    if len(items) == before:
        return False
    _write_user_favorites(ws, row_idx, items)
    return True
...
# Важливо: історична колонка payout_rate тепер зберігає ФІКСОВАНУ
# винагороду за одного оплаченого реферала, а не відсоток.
def upsert_ref_stats(ref_code: str, payout_rate: float = 50.0, default_price: float = 50.0) -> None:
    users_ws = _ws(USERS_WS)
    ref_ws = _ws(REF_WS)

    users = users_ws.get_all_records()
    _ensure_user_column(users_ws, "ref_reward_granted")
    invited = [u for u in users if str(u.get("ref_from", "")).strip() == str(ref_code)]
    paid_count = 0
    for u in invited:
        reward_granted = _to_bool(u.get("ref_reward_granted", ""))
        payment_id = str(u.get("payment_id", "")).strip()
        legacy_paid = bool(payment_id) and not payment_id.startswith("admin_")
        if reward_granted or legacy_paid:
            paid_count += 1
    revenue_sum = paid_count * float(default_price)
    payout_rate = float(payout_rate)
    payout_due = round(paid_count * payout_rate, 2)
    ...
    for r in rows:
        if str(r.get("ref_code", "")).strip() == str(ref_code):
            ...
            return {
                ...
                "payout_rate": f(r.get("payout_rate", 50)),
                "payout_due": f(r.get("payout_due", 0)),
                "payout_paid": f(r.get("payout_paid", 0)),
                "payout_left": f(r.get("payout_left", 0)),
            }
    return {"paid_count": 0, "revenue_sum": 0.0, "payout_rate": 50.0, "payout_due": 0.0, "payout_paid": 0.0, "payout_left": 0.0}


def mark_referral_reward_granted(user_id: str) -> None:
    ws = _ws(USERS_WS)
    row_idx = _find_row_by_user_id(ws, str(user_id))
    if not row_idx:
        return
    col = _ensure_user_column(ws, "ref_reward_granted")
    ws.update_acell(_a1(row_idx, col), "TRUE")


def referral_reward_was_granted(user_id: str) -> bool:
    ws = _ws(USERS_WS)
    row_idx = _find_row_by_user_id(ws, str(user_id))
    if not row_idx:
        return False
    col = _ensure_user_column(ws, "ref_reward_granted")
    row = ws.row_values(row_idx)
    if len(row) >= col and _to_bool(row[col - 1]):
        return True
    payment_id = str(row[11]).strip() if len(row) >= 12 else ""
    return bool(payment_id) and not payment_id.startswith("admin_")
