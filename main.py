...
from seed_cache import ensure_seed_cache
...
from background_indexer import get_indexer_status, start_background_indexer, run_index_once
...
from gsheets import (
    ...
    find_user_id_by_username, get_user_snapshot, get_subscription_gateway_meta,
    mark_wayforpay_recurring_status, get_user_favorites, remove_user_favorite, toggle_user_favorite,
    mark_referral_reward_granted, referral_reward_was_granted,
)
from free_access import free_views_used_up, has_active_subscription as has_paid_access, rent_access_blocked
from favorites import get_remembered_card

# === Основні словники ===
user_sub = {}          # user_id -> bool
pending_orders = {}    # user_id -> orderReference
payment_orders = {}    # orderReference -> user_id
admin_pending_action = {}  # admin_user_id -> action key
...
def _wayforpay_merchant_password() -> str:
    return _env("WAYFORPAY_MERCHANT_PASSWORD")


def _build_order_reference(user_id: int | str) -> str:
    return f"FF-{user_id}-{int(time.time() * 1000)}-{os.urandom(2).hex()}"
...
def _wayforpay_recurring_manage_ready() -> bool:
    return bool(_env("WAYFORPAY_MERCHANT_ACCOUNT") and _wayforpay_merchant_password())
...
def _wayforpay_regular_api(request_type: str, order_reference: str) -> dict:
    if not _wayforpay_recurring_manage_ready():
        raise RuntimeError("WayForPay recurring cancellation is not configured")
    request_type = str(request_type or "").strip().upper()
    order_reference = str(order_reference or "").strip()
    if not request_type or not order_reference:
        raise ValueError("request_type and order_reference are required")

    merchant_account = _env("WAYFORPAY_MERCHANT_ACCOUNT")
    merchant_password = _wayforpay_merchant_password()
    payload = {
    ...
    data = response.json()
    logging.info("[WayForPay regularApi %s] %s", request_type, json.dumps(data, ensure_ascii=False))
    return data


def _wayforpay_remove_recurring(order_reference: str) -> dict:
    return _wayforpay_regular_api("REMOVE", order_reference)


def _wayforpay_recurring_status(order_reference: str) -> dict:
    return _wayforpay_regular_api("STATUS", order_reference)


def _wayforpay_remove_succeeded(result: dict) -> bool:
    status = str(result.get("status", "")).strip().lower()
    reason_code = str(result.get("reasonCode", "")).strip()
    reason = str(result.get("reason", "")).strip().lower()
    if status == "ok":
        return True
    if status in {"removed", "completed"}:
        return True
    if reason_code in {"4100", "1100"}:
        return True
    if "ok" in reason or "removed" in reason:
        return True
    return False
...
def _activate_paid_subscription(user_id: str, order_reference: str, payment_id: str | None = None) -> None:
    ...
    snapshot_before = get_user_snapshot(str(user_id)) or {}
    reward_already_granted = referral_reward_was_granted(str(user_id))
    ...
    snapshot = get_user_snapshot(str(user_id)) or {}
    upsert_ref_stats(str(user_id), payout_rate=REFERRAL_PAYOUT_UAH, default_price=_price_value())
    ref_from = str(snapshot_before.get("ref_from", "") or snapshot.get("ref_from", "") or "").strip()
    if ref_from and ref_from != str(user_id):
        if not reward_already_granted:
            mark_referral_reward_granted(str(user_id))
        upsert_ref_stats(ref_from, payout_rate=REFERRAL_PAYOUT_UAH, default_price=_price_value())
    try:
        send_step_photo(
            bot,
            int(user_id),
...
try:
    ...
    bot.set_my_commands([
        ...
        types.BotCommand("admin", "Адмін-панель"),
...
def _subscription_gate_inline() -> types.InlineKeyboardMarkup:
    kb = types.InlineKeyboardMarkup(row_width=1)
    kb.add(types.InlineKeyboardButton("💳 Оформити підписку", callback_data="subscribe_month"))
    kb.add(types.InlineKeyboardButton("🏠 Головне меню", callback_data="back_to_menu"))
    return kb


def _send_rent_locked_message(chat_id: int):
    bot.send_message(
        chat_id,
        "🔒 Безкоштовні 3 оголошення на цей місяць уже використані.\n\n"
        "Щоб продовжити пошук оренди, активуй підписку. Після оплати доступ до всіх варіантів відкриється без обмежень.",
        reply_markup=_subscription_gate_inline(),
    )
...
def get_admin_panel_keyboard():
    ...
    kb.add(
        types.InlineKeyboardButton("✅ Видати 30 днів", callback_data="admin_grant_30"),
        types.InlineKeyboardButton("❌ Забрати підписку", callback_data="admin_revoke_sub"),
    )
    kb.add(
        types.InlineKeyboardButton("ℹ️ Статус користувача", callback_data="admin_status_sub"),
        types.InlineKeyboardButton("🔄 Переіндексувати міста", callback_data="admin_reindex_all"),
    )
    return kb


def _resolve_admin_user_ref(raw_value: str) -> str | None:
    value = str(raw_value or "").strip()
    if not value:
        return None
    if value.startswith("@"):
        return find_user_id_by_username(value)
    if value.isdigit():
        return value
    return None


def _format_user_snapshot(snapshot: dict | None) -> str:
    if not snapshot:
        return "❌ Користувача не знайдено."
    active = "Так" if snapshot.get("subscribed") else "Ні"
    cancelled = "Так" if snapshot.get("sub_cancelled") else "Ні"
    return (
        "👤 <b>Статус користувача</b>\n\n"
        f"ID: <code>{html.escape(snapshot.get('id', '') or '')}</code>\n"
        f"Username: <code>{html.escape(snapshot.get('username', '') or '—')}</code>\n"
        f"Підписка активна: <b>{active}</b>\n"
        f"Автопродовження скасовано: <b>{cancelled}</b>\n"
        f"Ref code: <code>{html.escape(snapshot.get('ref_code', '') or '—')}</code>\n"
        f"Ref from: <code>{html.escape(snapshot.get('ref_from', '') or '—')}</code>"
    )
...
def start_handler(message):
    ...
    if len(args) > 1:
        ...
        if referrer_id != user_id:
            ...
            upsert_ref_stats(referrer_id, payout_rate=REFERRAL_PAYOUT_UAH, default_price=_price_value())
...
def rent_command(message):
    if rent_access_blocked(message.from_user.id):
        _send_rent_locked_message(message.chat.id)
        return
...
def admin_panel_command(message):
    if not _is_admin(message.from_user.id):
        bot.reply_to(message, "⛔️ У тебе немає доступу до адмін-панелі.")
        return
    admin_pending_action.pop(message.from_user.id, None)
    bot.send_message(
        message.chat.id,
        "🛠 <b>Адмін-панель</b>\n\n"
        "Тут можна вручну видати підписку, забрати її, перевірити статус користувача або запустити переіндексацію.",
        parse_mode="HTML",
...
def admin_panel_callbacks(call):
    if not _is_admin(call.from_user.id):
        bot.answer_callback_query(call.id, "⛔️ Немає доступу", show_alert=True)
        return

    action = call.data
    bot.answer_callback_query(call.id)
    if action == "admin_grant_30":
        admin_pending_action[call.from_user.id] = "grant_30"
        bot.send_message(call.message.chat.id, "Надішли ID користувача або @username, кому треба видати підписку на 30 днів.")
        return
    if action == "admin_revoke_sub":
        admin_pending_action[call.from_user.id] = "revoke"
        bot.send_message(call.message.chat.id, "Надішли ID користувача або @username, у кого треба забрати підписку.")
        return
    if action == "admin_status_sub":
        admin_pending_action[call.from_user.id] = "status"
        bot.send_message(call.message.chat.id, "Надішли ID користувача або @username, щоб перевірити статус.")
        return
    if action == "admin_reindex_all":
        threading.Thread(target=run_index_once, name="admin-reindex-all", daemon=True).start()
        bot.send_message(call.message.chat.id, "🔄 Переіндексацію всіх міст запущено у фоні.")


@bot.message_handler(func=lambda m: _is_admin(m.from_user.id) and bool(admin_pending_action.get(m.from_user.id)))
@safe_handler
def admin_panel_input(message):
    action = admin_pending_action.get(message.from_user.id)
    if not action:
        return

    user_ref = _resolve_admin_user_ref(message.text)
    if not user_ref:
        bot.reply_to(message, "❌ Не зміг розпізнати користувача. Надішли числовий ID або @username.")
        return
    ...
    if action == "grant_30":
        ...
        snapshot = get_user_snapshot(str(user_ref))
        admin_pending_action.pop(message.from_user.id, None)
        bot.reply_to(message, "✅ Підписку видано на 30 днів.\n\n" + _format_user_snapshot(snapshot), parse_mode="HTML")
        return
    ...
    if action == "revoke":
        ...
        set_subscription(
            ...
            price_uah=0,
            period_text="30d",
            payment_id=f"admin_revoke_{int(time.time())}",
        )
        admin_pending_action.pop(message.from_user.id, None)
        snapshot = get_user_snapshot(str(user_ref))
        bot.reply_to(message, "✅ Підписку вимкнено.\n\n" + _format_user_snapshot(snapshot), parse_mode="HTML")
        return

    if action == "status":
        admin_pending_action.pop(message.from_user.id, None)
        snapshot = get_user_snapshot(str(user_ref))
        bot.reply_to(message, _format_user_snapshot(snapshot), parse_mode="HTML")
...
def subscribe_month(call):
    ...
    ensure_user(str(uid), call.from_user.username or "")

    # Тимчасово зберігаємо активне замовлення
    order_reference = _build_order_reference(uid)
    pending_orders[uid] = {"type": "wfp_sub", "ts": int(time.time()), "order_reference": order_reference}
...
def cancel_subscription(call):
    ...
    gateway_meta = get_subscription_gateway_meta(uid)
    order_reference = str(gateway_meta.get("order_reference", "")).strip()

    if not _wayforpay_recurring_manage_ready():
        bot.answer_callback_query(call.id)
        bot.send_message(
            call.message.chat.id,
            "❌ Автоматичне скасування автосписання поки не налаштоване на стороні WayForPay. "
            "Не залишайте це так перед запуском: треба додати `WAYFORPAY_MERCHANT_PASSWORD`.",
        )
        return
    ...
    if not order_reference:
        ...
        bot.send_message(
            call.message.chat.id,
            "❌ Не вдалося знайти платіжний профіль для відключення автосписання. "
            "Напишіть у підтримку, щоб ми одразу відключили продовження вручну.",
        )
        return

    try:
        status_result = _wayforpay_recurring_status(order_reference)
        current_gateway_status = str(status_result.get("status", "")).strip()
        if current_gateway_status.lower() in {"removed", "completed"}:
            cancel_subscription_renewal(uid)
            mark_wayforpay_recurring_status(uid, current_gateway_status.lower())
            bot.answer_callback_query(call.id)
            bot.send_message(
                call.message.chat.id,
                "✅ Автосписання вже було відключене раніше. Доступ залишиться активним до кінця оплаченого періоду.",
            )
            return
        result = _wayforpay_remove_recurring(order_reference)
    except Exception:
        logging.exception("WayForPay recurring cancellation failed for user %s", uid)
        bot.answer_callback_query(call.id)
        bot.send_message(
            call.message.chat.id,
            "❌ Не вдалося підтвердити скасування автосписання у WayForPay. "
            "Спробуйте ще раз через кілька хвилин або напишіть у підтримку.",
        )
        return

    if not _wayforpay_remove_succeeded(result):
        bot.answer_callback_query(call.id)
        bot.send_message(
            call.message.chat.id,
            "❌ WayForPay не підтвердив відключення автосписання. "
            "Поки що підписку не скасовано, спробуйте ще раз або зверніться в підтримку.",
        )
        return

    cancel_subscription_renewal(uid)
    mark_wayforpay_recurring_status(uid, "removed")
...
def saved_flats_handler(message):
    if not has_paid_access(message.from_user.id):
        bot.send_message(
            message.chat.id,
            "🔒 Добірки доступні лише з активною підпискою.",
            reply_markup=_subscription_gate_inline(),
        )
        return
    favorites = get_user_favorites(str(message.from_user.id))
    if not favorites:
        bot.send_message(
            message.chat.id,
            "💾 У тебе поки що немає збережених оголошень.\n\n"
            "Відкрий будь-яке оголошення і натисни «⭐ В добірку».",
            reply_markup=get_main_menu(),
        )
        return

    bot.send_message(message.chat.id, f"💾 <b>Мої добірки</b>\n\nЗбережено: <b>{len(favorites)}</b>", parse_mode="HTML")
    for item in favorites[:20]:
        title = html.escape(str(item.get("title", "") or "Без назви"))
        ...
        kb = types.InlineKeyboardMarkup(row_width=1)
        if link:
            kb.add(types.InlineKeyboardButton("🔗 Переглянути", url=link))
        kb.add(types.InlineKeyboardButton("🗑 Видалити з добірки", callback_data=f"fav_remove:{item.get('key', '')}"))
        try:
            if image_url:
                bot.send_photo(message.chat.id, image_url, caption=caption, parse_mode="HTML", reply_markup=kb)
            else:
...
@bot.callback_query_handler(func=lambda c: (c.data or "").startswith("fav_toggle:"))
@safe_handler
def favorite_toggle_handler(call):
    bot.answer_callback_query(call.id)
    if not has_paid_access(call.from_user.id):
        bot.answer_callback_query(call.id, "Добірки доступні лише з підпискою", show_alert=True)
        return
    token = (call.data or "").split(":", 1)[1]
    card = get_remembered_card(token)
    if not card:
        bot.answer_callback_query(call.id, "Картка вже застаріла. Відкрий оголошення ще раз.", show_alert=True)
        return
    result = toggle_user_favorite(str(call.from_user.id), card)
    if result.get("saved"):
        bot.answer_callback_query(call.id, "⭐ Додано в добірку")
    else:
        bot.answer_callback_query(call.id, "🗑 Прибрано з добірки")


@bot.callback_query_handler(func=lambda c: (c.data or "").startswith("fav_remove:"))
@safe_handler
def favorite_remove_handler(call):
    bot.answer_callback_query(call.id)
    key = (call.data or "").split(":", 1)[1]
    if remove_user_favorite(str(call.from_user.id), key):
        try:
            bot.edit_message_reply_markup(call.message.chat.id, call.message.message_id, reply_markup=None)
        except Exception:
            pass
        bot.answer_callback_query(call.id, "Видалено з добірки")
    else:
        bot.answer_callback_query(call.id, "У добірці вже немає цього оголошення")
...
def how_referral_works(call):
    ...
    text = (
        ...
        f"За кожного користувача, який оформить підписку через вашу реферальну лінку, "
        f"ви отримуєте *{REFERRAL_PAYOUT_UAH} грн*.\n\n"
