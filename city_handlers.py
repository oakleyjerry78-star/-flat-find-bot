...
from listing_cache import filter_cards_with_photos, query_cards_for_query, upsert_listings
...
def register_city_handlers(bot):
    ...
    def _dedupe_cards(cards: list[dict]) -> list[dict]:
        cards = filter_cards_with_photos(cards)
        seen, out = set(), []
        for c in cards:
    ...
    def send_listing(chat_id: int, show_cards: bool = True):
        listings = filter_cards_with_photos(user_listings.get(chat_id, []))
