    ...
    def _do_search_and_send_office(chat_id, loading_msg_id=None):
        try:
            ...
            if cached:
                ...
                return
            listings_first = provider.search({**q, "max_pages": 1}) or []
            if listings_first:
                try:
            ...
            if cards_first:
                ...
                safe_send_message(
                    chat_id,
                    "⏳ Поки не знайшов офісів у швидкому режимі. Я вже поставив цей пошук у пріоритет — спробуй ще раз за хвилину.",
                )
        ...
        for listing in listings[start:end]:
            try:
                ...
                if (not view_state.get("subscribed")) and int(view_state.get("remaining", 0) or 0) <= 0:
                    if start > 0:
                        markup.add(types.InlineKeyboardButton("◀️ Назад", callback_data="prev_page_office"))
