...
DB_PATH = Path(os.getenv("OLX_CACHE_DB_PATH", str(Path(__file__).with_name("olx_cache.sqlite3"))))
...
def _row_search_blob(row: sqlite3.Row) -> str:
    raw = _loads(row["raw_json"], {}) if row["raw_json"] else {}
    extra = raw.get("extra") if isinstance(raw, dict) else {}
    parts = [
        row["district"] or "",
        row["title"] or "",
        raw.get("district", "") if isinstance(raw, dict) else "",
        raw.get("address", "") if isinstance(raw, dict) else "",
        extra.get("location_text", "") if isinstance(extra, dict) else "",
    ]
...
def query_cards(
    *,
    ...
    if require_photos:
        where.append("photos_json IS NOT NULL AND photos_json!='[]'")
    if city:
    ...
    with _LOCK, _connect() as conn:
        for row in conn.execute(sql, params):
            if district_norm:
                haystack = _row_search_blob(row)
                if not any(d in haystack for d in district_norm):
                    continue
            key = row["listing_id"] or row["url"]
            if not key or key in seen:
                continue
            seen.add(key)
            photos = [_normalize_photo_url(p) for p in _loads(row["photos_json"], []) if p]
            if require_photos and not photos:
                continue
            price = row["price_uah"]
            out.append(
                {
...
def card_has_photos(card: dict[str, Any]) -> bool:
    photos = card.get("img_urls") or card.get("photos") or card.get("images") or []
    if isinstance(photos, dict):
        photos = list(photos.values())
    if isinstance(photos, str):
        photos = [photos]
    return any(_normalize_photo_url(url) for url in (photos or []))


def normalize_card_photos(card: dict[str, Any], limit: int = 6) -> dict[str, Any]:
    photos = card.get("img_urls") or card.get("photos") or card.get("images") or []
    if isinstance(photos, dict):
        photos = list(photos.values())
    if isinstance(photos, str):
    ...
    for url in photos or []:
        photo_url = _normalize_photo_url(url)
        if photo_url:
            normalized.append(photo_url)
    card["img_urls"] = normalized[:limit]
    return card


def filter_cards_with_photos(cards: list[dict[str, Any]], limit: int | None = None) -> list[dict[str, Any]]:
    out: list[dict[str, Any]] = []
    seen: set[str] = set()
    for card in cards or []:
        normalized = normalize_card_photos(dict(card))
        if not normalized["img_urls"]:
            continue
        key = (normalized.get("_key") or normalized.get("link") or "").strip()
...
def prepare_cards_for_display(cards: list[dict[str, Any]], limit: int | None = None) -> list[dict[str, Any]]:
    """Normalize card photos and dedupe cards, but keep text-only listings too."""
    out: list[dict[str, Any]] = []
    seen: set[str] = set()
    for card in cards or []:
        normalized = normalize_card_photos(dict(card))
